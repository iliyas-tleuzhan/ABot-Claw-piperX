...
rear drivers, the combined `/joint_states`, the cameras, and the root TF tree.
Do not start another driver, RealSense node, robot-state publisher, or MoveIt
instance when those nodes already exist. The combined MoveIt/TF names are
...
Perception must use the combined TF chain:
`base_link -> front_piper_flange_link -> front_camera_color_optical_frame`.
Marker planning must use the existing `/front_piper` MoveIt instance with the
combined `/robot_description` and matching integrated front SRDF. Never pair
the old standalone AgileX SRDF (`base_link`, `tcp_link`, raw `joint1..6`) with
the combined dual-arm URDF, which causes MoveIt entity-resolution failures.
